...
- **The release workflow's `npm run make` failing on macOS is a single point of failure for that OS's whole job**, including the zip: Forge's `make()` runs all matched makers together and an unhandled rejection from any one of them (here, DMG) aborts the process before an in-flight sibling maker (zip) finishes writing its output — confirmed locally (a combined run produced no files in `out/make/zip` at all, while a zip-only run produced the file cleanly). Not worth engineering around speculatively: the one cause found here is sandbox-specific and shouldn't occur on GitHub's runners. Flagged here so a real DMG failure on CI reads as "investigate," not "mysterious zip loss."
