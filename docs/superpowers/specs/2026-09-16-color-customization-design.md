...
Four tokens have a translucent (`rgba(...)`, not solid hex) default in at least one mode: `line-hairline`, `line-strong`, `surface-overlay` (both modes), and `surface-accent-soft` (dark only). `<input type="color">` only accepts/emits opaque 6-digit hex, so these can't be edited directly as a raw picker value — but the transparency doesn't need to be lost. Each of these four gets a fixed, non-editable alpha constant per mode (taken from its current default — e.g. `line-hairline` dark is `.10`, `surface-overlay` light is `.55`), stored alongside the token list. The picker still lets the user choose only the hue/lightness (shown as if opaque); on change, the app combines the picked RGB with that token's fixed alpha into an `rgba()` string, which is what actually gets applied and stored. This is the same idea as "use opacity" but implemented as a color value (`rgba()`), not the CSS `opacity` property — `opacity` would dim the whole element the token is applied to (e.g. an entire bordered card), not just contribute an alpha channel to that one border color, which is wrong for a border/overlay/tint color sitting next to or on top of other content. Net effect: no functionality lost, at the cost of these 4 tokens not being alpha-tunable by the user (matches the design's own intent — these were never meant to carry arbitrary opacity anyway).
...
- For the 4 translucent-by-default tokens (see "Scope"), the conversion happens entirely in this component, so the rest of the system (storage, IPC, CSS injection) stays uniform — every stored override is always a plain, ready-to-use CSS color string. On save: combine the picker's opaque hex with the token's fixed alpha constant into an `rgba()` string before calling `setPaletteOverride`. On load: strip the alpha from the effective value (override or default) to get the opaque hex the `<input>` needs to display.
...
2. The 4 translucent-by-default tokens (`line-hairline`, `line-strong`, `surface-overlay`, `surface-accent-soft`) keep their transparency (see "Scope" above) but aren't alpha-tunable — the user picks hue only, alpha stays at the design's original fixed value per mode.
